...
现代嵌入式编程第37课，Miro Samek 继续讲 state machines，不过重点从前两课的 event-driven state machine 扩展到了“软件里其他常见的状态机类型”。Samek 这一课做了两件很重要的事：
...
课程一开始先回到历史。状态机并不是先在软件领域兴起的，而是源于 1950 年代的数字电路设计，尤其是 George Mealy 和 Edward Moore 在 Bell Labs 的经典论文。
![pragramming trend](./images/programming_trends.png)
...
![combinational circuit](./images/combinational_circuits.drawio.svg)
...
接着课程举了一个更能体现“状态”必要性的例子：检测一个输入信号的 rising edge。
...
![memory](./images/circuits_with_memory.drawio.svg)
Samek 给出的电路例子如上，状态是由 D flip-flop 保存的，而 flip-flop 又受时钟驱动。这个时钟决定系统什么时候采样输入、什么时候更新状态。
...
顺带地，asynchronous circuit 当然也可能存在，但它们更容易遭遇由于传播延迟引起的硬件竞态，所以现实中的数字系统几乎都采用同步设计。
...
rising edge检测电路说明了这种形式的优势和代价：它可以更快地对输入变化作出响应，但也因为输出直接依赖输入，更容易受到同步边界附近竞争关系的影响。

## Moore 状态机：输出只依赖当前状态

与 Mealy 相对的是 Moore state machine。
![memory](./images/circuits_with_memory2.drawio.svg)
...
特别指出，Moore 机往往需要更多状态，才能完成与 Mealy 机相同的功能。但它的分离性更好，在硬件中也常常更稳健。
...
课程倾向于用 input-driven 或 polled 这类称呼，因为它们更准确地反映了其本质：状态机本身必须去 poll 输入，并在 guard condition 中“发现”真正有意义的事件。
...
1. 在 superloop 中先构造输入快照（buffered inputs）。
2. 用 guard 检测按钮边沿（`prev_button` 与当前 `button`）。
3. 说明这些输入为何是“异步来源”（来自 ISR 更新的 tick 与 GPIO 读取）。

代码来源：`lesson-37/tm4c123-keil`。

### 1. 在状态机入口构造一次输入快照（`main.c`）
...
while (1) {
	...
	e = &evt;
	/* 本轮 RTC 处理只使用 e->now 与 e->button */
}
```

这段代码正是 Samek 在课里强调的 buffering 思路：在一次处理开始时先采样，再在同一轮逻辑中反复使用同一个快照。

### 2. guard 检测按钮边沿（`main.c`）

```c
static uint8_t prev_button = 1U; /* SW1 is inactive high */

/* ...OFF_STATE 或 ON_STATE 内... */
if ((prev_button != 0U) && (e->button == 0U)) { /* depressed? */
	BSP_ledBlueOn();
...
这里的 guard 并不是事件队列里的离散信号，而是通过“前值 vs 当前值”在 input-driven 状态机内部识别出来的 proto-event（按下/松开）。

### 3. 异步输入来源示例（`bsp.c` / `bsp.h`）
...
`l_tickCtr` 在 `SysTick_Handler()` 中异步更新，`SW1` 也可能在任意时刻变化，这正是本课讨论 input-driven 状态机时序风险的现实来源。
